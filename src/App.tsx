...
import { signersAdd, signersAddList, signersClear } from './store/actions/signers';
...
import {isWeb3Injected, web3Accounts, web3Enable} from "@reef-defi/extension-dapp";
...
const accountToSigner = (provider: Provider, sign: InjectedSigner) => async (account: InjectedAccountWithMeta): Promise<RemixSigner> => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        .catch(async (err) => {
          throw new Error(`There was an error when connecting to newtork ${network.name}... Check network status!`);
        })
        const inj = await web3Enable("reef-remix-plugin");
        if (inj.length === 0) {
          throw new Error("Reef extension not detected. Please install it on link: https://github.com/reef-defi/browser-extension")
        }
        const accountsInj = await web3Accounts();
        const accounts = await Promise.all(accountsInj.map(accountToSigner(newProvider, inj[0].signer)));
